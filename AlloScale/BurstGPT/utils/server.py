...
from .vllm import vllm_inference_call_server
from .lightllm import lightllm_inference_call_server
from .sglang import sglang_inference_call_server


class ServerBase:
    def __init__(self, model_path, data_path, backend, device, log_path, config):
        self.config = config
        ...
        if self.config.prompt_config.get('use_burstgpt'):
            self.conv_or_api = self.config.prompt_config['conv_or_api']
            chunk_iter = pd.read_csv(self.config.prompt_config['burstgpt_path'], chunksize=20)

            chunks = []
            conv_or_api_str = 'Conversation log' if self.conv_or_api == 'conv' else 'API log'
            print(conv_or_api_str)
...
class QueryGenerator:
    def __init__(self, inputs, model_path, qps=1.0, trace=None, conv_or_api=None, scale=1):
        self.inputs = inputs
        self.qps = qps
        self.query_time = 0
    ...
    def get_query(self):
        ...
        if self.trace is None or self.query_id >= len(self.trace):
            if self.query_id >= 0 and self.gamma_step < len(self.shape_list) - 1:
                ...
                print("self.gamma_step: ", self.tokenizer)
        ...
        if 0 <= self.query_id <= len(self.trace) - 1 if self.trace is not None else False:
            if self.query_id % 10 == 0:
                self.gamma_shape_dict[self.query_id] = {
                    "query_id": self.query_id,
                    "query time": time.monotonic() + self.query_time,
            ...
            if self.query_id == len(self.trace) - 1:
                ...
                if self.trace is None:
                    current_time_str = f"gamma_zipf_{current_time}.json"
                else:
                    current_time_str = f"trace_{self.conv_or_api}_{current_time}.json"
                with open(current_time_str, "a") as f:
                    json.dump(self.gamma_shape_dict, f)

        idx_info = self.prefill_idx[sampled_prompt_len][sampled_output_len]
        prompt_data = self.inputs[idx_info]

        prompt_text = prompt_data[0]
        prompt_len = prompt_data[1]
        output_len = prompt_data[2]

        print(f"Prompt Length in Tokens: {len(self.tokenizer.encode(prompt_text))}")
...
class ServerOnline(ServerBase):
    ...
    async def issue_queries(self):
        ...
        for _ in range(surplus_prompts_num):
            ...
            _prompt, in_num, _out_num, sampled_in_num, sampled_out_num, _, _sleep_time = self.queries.get_query()
            request_out_num = 1
            ...
            if self.backend == "sglang":
                ...
                print("Inference backend must be specified")
