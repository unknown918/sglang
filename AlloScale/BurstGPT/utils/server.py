...
import random
import pandas as pd
from wrapper import vllm_inference_call_server, lightllm_inference_call_server, sglang_inference_call_server
...
class ServerOnline(ServerBase):
    class _Query(object):
        ...
        def get_query(self):
            ...
            return [self.inputs[self.prefill_idx[sampled_prompt_len][sampled_output_len]][0], prompt_len,
                    self.max_gen_len, sampled_prompt_len, self.max_gen_len, delta_time, self.query_time]

    def __init__(self, model_path, data_path, backend="sglang",
                ...
                _task_list.append(
                    asyncio.create_task(
                        sglang_inference_call_server(
                            _prompt, in_num, out_num, sampled_in_num, sampled_out_num,
